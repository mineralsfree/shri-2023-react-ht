...
export const DropDown = (props: DropDownProps) => {
    ...
    return (<div ref={dropDownRef} onClick={() => setExpanded(!expanded)} className={classNames(styles.input, {[styles.expanded]: expanded})}>
        <p>{selectedLabel ? selectedLabel.label : placeHolder}</p>
        <ExpandButton size={20} fill='#999FA6' expanded={expanded}/>
        {expanded && dropDownRef.current && ReactDOM.createPortal(
            <div
            ...
            >
                {optionsWithDefault.map(option => {
                    ...
                    }} key={option.value + option.label} className={styles.option}>{option.label}</div>)
                })}
            </div>,
            document.body
        )}
