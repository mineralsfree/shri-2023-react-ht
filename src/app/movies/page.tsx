...
export default function MoviesPage() {
  ...
  let mergedMovies: MovieWithCinema[] | undefined = useMemo(
    () =>
      cinemas &&
      movies?.map((movie: Movie) => {
        return {
          ...movie,
          cinema: cinemas?.find(
            (cinema: Cinema) => cinema?.movieIds.includes(movie.id),
          )?.name,
        };
      }),
    [movies, cinemas],
  ...
  const genres: string[] = useMemo(
    () =>
      (movies &&
        Array.from(
          movies.reduce((acc, curr) => acc.add(curr.genre), new Set<string>()),
        )) ||
      [],
    [movies],
  );
      ...
      <div className={styles.moviePageContainer}>
        ...
        <div className={styles.movieFilterContainer}>
          {mergedMovies.length > 0 ? (
            mergedMovies.map((movie: MovieWithCinema) => (
              <MovieCard
                variant={"small"}
            ...
            <h2>По критериям поиска ничего не найдено</h2>
          )}
        </div>
      </div>
    )
