...
const transformResponse = <T extends Movie | Movie[]>(response: T): T => {
    if (Array.isArray(response)) {
        return response.map((movie: Movie): Movie => ({...movie, genre: genreMap[movie.genre]})) as T;
    } else {
...
export const movieApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getMovieComments: builder.query<Review[], string>({query: (movieId) => `reviews?movieId=${movieId}`})
    })
})
export const {useGetMoviesQuery, useGetMovieQuery, useGetMovieCommentsQuery} = movieApi;
